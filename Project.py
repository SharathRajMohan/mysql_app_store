...
def top_chart():
	...
	mycursor.execute("SELECT * FROM app WHERE download_count > 50000 AND rating > 4 ORDER BY rating")
	row=mycursor.fetchall()
	mydb.commit()
...
def show_all():
	...
	mycursor.execute("CREATE VIEW show_all AS SELECT app_name, cost, size, category_name, rating, download_count FROM app, category where app.c_id = category.c_id")
